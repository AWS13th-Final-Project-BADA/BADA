...
### Dev/Prod 환경 분리 후속 과제

Partial backend config로 dev/prod state를 분리하고 prod 환경(`bada-prod-*`, `prod.badasoft.com`)을 실제로 올렸다(트러블슈팅 5-5 참고). 환경 뼈대는 섰지만 prod를 실운영 수준으로 쓰려면 아래가 남아 있다.

- **prod 전용 CD 파이프라인 부재**: 현재 prod ECS는 임시로 dev ECR(`bada-dev-backend:latest`)을 참조하고, 배포도 수동이다. `deploy-dev.yml` 등 워크플로가 `bada-dev-*` 클러스터·역할 ARN을 하드코딩하고 있어 prod 자동 배포 경로가 없다. **개선 방향(선택)**: 워크플로를 환경 매트릭스로 파라미터화하고 prod ECR·전용 배포 역할을 추가.
- **prod OAuth 콜백 미등록**: `https://api.prod.badasoft.com/auth/{google|kakao|naver}/callback`을 구글/카카오/네이버 콘솔에 등록해야 prod 로그인이 동작한다(외부 콘솔 의존, 인증·모바일 담당).
- **rollback용 Single-AZ DB 잔존**: Multi-AZ cutover 후 남긴 rollback DB(dev `bada-dev-postgres`, prod의 rehearsal 구조 부산물)를 안정화 확인 뒤 final snapshot 후 정리해야 한다. prod는 primary `multi_az`를 변수화하면 rehearsal 부산물 자체를 없앨 수 있다.
- **prod 승인·권한 분리 미비**: 현재 dev/prod apply를 같은 담당·같은 role로 수행한다. 실운영 전환 시 prod apply role 분리, 배포 승인 게이트, prod 전용 알람 SNS 수신자 분리가 필요하다.

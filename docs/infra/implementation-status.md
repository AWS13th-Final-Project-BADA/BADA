...
| Backend Task Definition | 완료 | `bada-dev-backend:79`, Backend SDK import 오류 해소 전까지 X-Ray 비활성 |
| Frontend Task Definition | 제거됨 | `frontend_enabled=false` 적용 이후 미운영 |
| Worker Task Definition | 완료 | `DATABASE_URL`, S3 Report Bucket, SQS, AWS Provider, X-Ray daemon sidecar 적용 |
| Backend ECS Service | 실행 중 | `desired_count=1` |
| Frontend ECS Service | 제거됨 | `https://badasoft.com`은 Backend 폴백 |
| Worker ECS Service | 실행 중 | `desired_count=1`, 분석·STT 메시지 처리 검증, `xray-daemon` RUNNING |

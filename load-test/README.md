...
> 📄 **성능 검증 Case Study**: [performance-case-study.md](./performance-case-study.md) — 운영 dev/prod와 분리된 perf 환경에서 1,000 VU 부하 기준 Backend CPU 병목을 발견하고, 리소스 조정 후 p95 60,007ms → 96ms, 처리량 약 29배 개선을 검증한 결과 문서(그래프 포함).

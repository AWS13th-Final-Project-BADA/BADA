...
|---------|------|----------|------------|-----------|------|
| 1. CPU 스트레스 | `k6/backend-autoscaling.js` | 합성(폐쇄형, `/health`) | **오토스케일링 메커니즘이 실제 발동** | Backend CPU 70% | 0 |
| 2. 사용자 여정 | `k6/backend-journey.js` | 현실적 읽기(인증+DB+번역) | **동시 사용자 하 지연/처리량/에러율**, 실부하는 I/O 바운드임을 확인 | (대개 미발동 — I/O 바운드) | 0 |
| 3. Worker 적체 | `sqs/fill_backlog.py` | 현실적 서지(SQS 큐 적체) | **비동기 파이프라인 Worker scale-out** | Worker SQS backlog | 0 (bogus case_id) |
...
## 1. CPU 스트레스 (Backend 스케일 메커니즘 증명)
```bash
k6 run load-test/k6/backend-autoscaling.js          # VUS=40, 폐쇄형
```
확인: Grafana Backend CPU% 70%↑ + RunningTaskCount 1→2→3, `describe-scaling-activities`(backend).
...
## 3. Worker SQS 적체 (비동기 서지, Worker 스케일)
```bash
# PowerShell: $env:AWS_PROFILE="bada-team"
python load-test/sqs/fill_backlog.py --count 6000    # 적체 투입 (bogus case_id, 비용 0)
```
- 원리: 없는 case_id → 워커가 첫 DB 조회에서 실패(Bedrock 미호출). 테스트 창엔 DLQ로 안 감(visibility 900s).
- 확인: Grafana Worker "SQS 대기 메시지" 상승 + Worker RunningTaskCount 1→2→3, `describe-scaling-activities`(worker).

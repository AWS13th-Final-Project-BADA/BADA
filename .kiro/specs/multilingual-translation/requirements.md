...
- **SUPPORTED_LANGUAGES**: 시스템이 지원하는 언어 코드 집합 (ko, vi, en, id, km, ne, th, ja)
...
1. WHEN the target language is "vi", "en", "id", "th", or "ja", THE Translator SHALL use the direct strategy (ko→target via Amazon Translate)
...
1. THE LanguageConfig SHALL define exactly eight supported languages: ko, vi, en, id, km, ne, th, ja
...
4. THE LanguageConfig SHALL assign "direct" strategy to vi, en, id, th, and ja
...
### Requirement 10: 리포트 실시간 다국어 조회

**User Story:** As a 외국인 노동자, I want 분석을 다시 실행하지 않고도 리포트를 모국어로 볼 수 있다, so that 언어를 바꿔가며 내용을 확인할 수 있다.

#### Acceptance Criteria

1. WHEN a user requests the report with a non-ko lang parameter, THE System SHALL translate all dynamic content (timeline, deductions, translation pairs, missing evidence) at view time using Amazon Translate
2. WHEN a user requests the report with lang=ko, THE System SHALL display all content in the original Korean without any translation API calls
3. THE System SHALL translate report UI elements (section headings, labels, disclaimer) based on the lang parameter using pre-defined locale strings
4. THE System SHALL NOT require re-running the analysis to view the report in a different language
5. THE System SHALL store analysis results in Korean only, and translate on-demand at report view time

### Requirement 11: 제출용 PDF는 한국어 고정

**User Story:** As a 외국인 노동자, I want 인쇄/PDF 저장 시 한국어 원본 리포트를 받고 싶다, so that 공공기관(고용노동부)이나 법무법인에 제출할 수 있다.

#### Acceptance Criteria

1. WHEN the user clicks the print/PDF button in the report, THE System SHALL open the Korean (lang=ko) version of the report
2. THE print/PDF button SHALL always link to the report with lang=ko regardless of the current viewing language
3. THE Korean report SHALL contain all original source text without any translation applied to dynamic content

### Requirement 12: 번역 모드 독립 설정

**User Story:** As a 개발자, I want 번역 기능만 AWS 모드로 전환할 수 있다, so that LLM/OCR이 미구현이어도 번역 테스트를 할 수 있다.

#### Acceptance Criteria

1. THE System SHALL support a TRANSLATE_MODE environment variable that overrides PROVIDER_MODE for translation only
2. WHEN TRANSLATE_MODE=aws is set, THE System SHALL use AmazonTranslator regardless of PROVIDER_MODE value
3. WHEN TRANSLATE_MODE is not set, THE System SHALL fall back to the value of PROVIDER_MODE

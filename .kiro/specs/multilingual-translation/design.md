...
핵심 원칙은 **원문 항상 병기**(side-by-side)로, 번역 오류 시 원문 확인이 가능해야 한다. 정적 UI 텍스트는 next-intl JSON locale 파일로 관리하고, 동적 분석 결과는 워커 파이프라인 3단계에서 Amazon Translate를 호출하여 런타임 번역한다.

...
function renderChat(){
  ...
  list.innerHTML=S.chat.map(m=>{
    ...
    return `<div class="chat-bubble bot">${esc(m.text)}${meta}${next}${disclaimer}</div>`;
  }).join("");
...
async function sendChatMessage(event){
  ...
  try{
    ...
    S.chat.pop();
    S.chat.push({role:"bot",text:res.answer,meta:res,next_actions:res.next_actions||[],disclaimer:res.disclaimer});
  }catch(e){
    S.chat.pop();

...
def report(case_id: str, db: Session = Depends(get_db)):
    ...
    trs = "".join(
        f"<tr><td>{p.get('source_text','')}</td><td>{p.get('translated_text','')}</td><td>{p.get('evidence_type') or ''}</td></tr>"
        for p in r.get("translations", []))

    miss = "".join(f"<li>[{m.get('item','')}] {m.get('reason','')}</li>" for m in r.get("missing", []))
    gps = r.get("gps") or {}
...
<div class="disc">{narrative.get('disclaimer','')}</div>
...
<div class="sum">{narrative.get('summary') or '(요약 없음)'}</div>
...
{"".join(f'<p class="muted" style="font-size:12px">· {n}</p>' for n in wage.get('notes', []))}
